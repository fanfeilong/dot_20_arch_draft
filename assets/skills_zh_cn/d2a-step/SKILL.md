...
5. 正文必须自动分行，单行不超过 80 字；超长内容要拆成多行短句。
...
state: 分析 3/7｜核心对象 -> 分析 4/7｜状态机 · 继续请使用 $d2a-step
--------------------------------------------------
```
...
==================================================
【分析 1/7｜边界】n8n_d2a
next: d2a-arch-1-project-scope -> docs/1.架构拆解/01_边界.md
==================================================

- 即将执行：d2a-arch-1-project-scope（文件：docs/1.架构拆解/01_边界.md）。
- 请确认是否继续本动作。（是/否）
[human_in_loop]

--------------------------------------------------
done: 已完成下一个动作的执行前确认
state: 分析 1/7｜边界 -> 分析 1/7｜边界 · 继续请使用 $d2a-step
--------------------------------------------------
```
...
8. 分析主链顺序固定为：
   ...
   - 只有 `d2a-arch-7-overview` 完成后，才进入 `d2a-challenge-architecture`。
